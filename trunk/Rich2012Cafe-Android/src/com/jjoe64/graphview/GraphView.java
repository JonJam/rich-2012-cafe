...
abstract public class GraphView extends LinearLayout {
	...
	static public class GraphViewData {
		...
		public GraphViewData(Date valueX, double valueY) {
			super();
			this.date = valueX;
			this.valueY = valueY;
		}
	...
	private GraphViewData[] _values(int idxSeries) {
		...
		if (viewportStart == 0 && viewportSize == 0) {
			...
			for (int i=0; i<values.length; i++) {
				if (values[i].date.getTime() >= viewportStart) {
					if (values[i].date.getTime() > viewportStart+viewportSize) {
	...
	protected String formatDate(long value){
		Date temp = new Date(value);
	...
	private String[] generateHorlabels(float graphwidth) {
		...
		long min = getMinX(false);
		long max = getMaxX(false);
		for (int i=0; i<=numLabels; i++) {
			labels[i] = formatDate(min + ((max-min)*i/numLabels));
		}
		return labels;
	}
	...
	private long getMaxX(boolean ignoreViewport) {
...
//			return highest;
	...
	private long getMinX(boolean ignoreViewport) {
		// if viewport is set, use this
		if (!ignoreViewport && viewportSize != 0) {
